...
public class maximumScore3414 {
  ...
  public int[] maximumWeight(List<List<Integer>> intervals) {
    ...
    for (int i = 0; i < n; i++) {
      List<Integer> ls = intervals.get(i);
      arr[i][0] = ls.get(0); //start
      arr[i][1] = ls.get(1); //end
      arr[i][2] = ls.get(2); //weight
      arr[i][3] = i; //original index
    }
    //now arr looks like: [start,end, weight,original_index]
    //step 2: sort the array on the basis or 'start'
    Arrays.sort(arr, (a, b) -> Integer.compare(a[0], b[0]));
    return new int[] { 1, 10 };
  }

  public int[] maximumWeight2(List<List<Integer>> intervals) {
    int n = intervals.size();
    int[][] arr = new int[n][4];
    ...
    for (int i = 0; i <= n; i++) for (int k = 0; k <= 4; k++) dpList[i][k] =
      new ArrayList<>();
  ...
  private boolean isLexicographicallySmaller(List<Integer> a, List<Integer> b) {
    ...
    for (int i = 0; i < size; i++) if (!a.get(i).equals(b.get(i))) return (
      a.get(i) < b.get(i)
    );
